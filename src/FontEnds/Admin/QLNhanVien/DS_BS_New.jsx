...
const DS_BS = ({ setContext }) => {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setError(err.message || 'Không thể xóa bác sĩ');
    }
  };

  const columns = [
    { key: 'stt', label: 'STT', render: (row, index) => (currentPage - 1) * 10 + index + 1 },
    { key: 'staff_code', label: 'Mã BS' },
    { key: 'full_name', label: 'Họ tên' },
    { key: 'specialization', label: 'Chuyên khoa' },
    ...
    <div className="px-4 py-3">
      ...
      <div className="bg-white p-5 rounded-lg shadow-sm mb-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">Quản lý thông tin bác sĩ</p>
          </div>
          <div className="flex gap-2">
            <Button
              variant="outline"
            ...
            <Button
              variant="primary"
              onClick={() => setContext({ view: "Thêm NV", role: "doctor" })}
            >
              Thêm Bác Sĩ
            </Button>
          </div>
      ...
      <div className="bg-white p-4 mb-4 rounded-lg shadow-sm">
        <div className="flex gap-4 items-end">
          <div className="flex-1">
            <Input
              label="Tìm kiếm"
              placeholder="Tìm theo tên, SĐT, email, mã bác sĩ..."
              value={search}
      ...
      <div className="bg-white p-3 mb-4 rounded-lg shadow-sm flex items-center justify-between">
        <div className="text-sm text-gray-600">
          Tìm thấy <span className="font-semibold">{staff.length}</span> bác sĩ
        </div>
        <div className="flex gap-3 items-center">
          ...
          >
            ...
            <option value="full_name">Tên bác sĩ</option>
            <option value="specialization">Chuyên khoa</option>
            <option value="staff_code">Mã bác sĩ</option>
          </select>
          <select
            value={sortOrder}
      ...
      <div className="bg-white rounded-lg shadow-sm overflow-hidden">
        {loading ? (
          ...
          <EmptyState
            ...
            description="Hãy thêm bác sĩ mới để bắt đầu"
            action={
              <Button variant="primary" onClick={() => setContext({ view: "Thêm NV", role: "doctor" })}>
